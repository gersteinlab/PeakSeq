...
t_enrichment_profile::t_enrichment_profile(char* _chr_id, t_fragment** fragments, int enrichment_fragment_length)
{
	...
	while(fragments[i_frag] != NULL)
        {
                t_fragment* cur_frag = fragments[i_frag];

                if(cur_frag->strand_char == 'F')
                {
						if(enrichment_fragment_length < cur_frag->sequenced_fragment_length)
						{
							this->add_fragment_ends(cur_frag->base_index, cur_frag->sequenced_fragment_length);
						}
						else
						{
							this->add_fragment_ends(cur_frag->base_index, enrichment_fragment_length);
						}
                ...
                else if(cur_frag->strand_char == 'R') // The base indices of reverse strand fragments point to 5' end of the tags.
                {
					...
					{
						this->add_fragment_ends(cur_frag->base_index - (enrichment_fragment_length - 1), enrichment_fragment_length);
					} 
                }
                else
                {
...
t_enrichment_profile::t_enrichment_profile(char* _chr_id, vector<t_fragment*>* fragments, int enrichment_fragment_length)
{
	this->chr_id = new char[strlen(_chr_id) + 2];
	strcpy(this->chr_id, _chr_id);
...
t_enrichment_profile::~t_enrichment_profile()
{
	...
	else if(this->frag_ends->size() == 0)
	{
		delete(this->frag_ends);
	}
	else
	{
		for(int i_frag_end = 0; i_frag_end < this->frag_ends->size(); i_frag_end++)
		{
					...
					int max_thresh)
{
	...
	for(int thresh = min_thresh; thresh <= max_thresh; thresh++)
	{
		...
		for(int i_peak = 0; i_peak < cur_threshold_peaks->size(); i_peak++)
		{
...
void t_enrichment_profile::dump_peak_profiles_per_list(vector<t_peak_region*>* peak_region_list, char* dump_dir, char* file_name_fmt_str)
{
    ...
    signed int height = 0;
        ...
        int i_end = 0;
    //for(int i_end = 0; i_end < this->frag_ends->size(); i_end++)
    while(i_end < this->frag_ends->size() &&
