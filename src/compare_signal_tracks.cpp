...
bool __DUMP_COMPARE_SIGNAL_TRACKS__ = false;
...
vector<t_peak_region*>* compare_signal_tracks(t_chip_seq_chr_data* cur_chr_data,
							...
							int max_thresh)
{
if(__DUMP_COMPARE_SIGNAL_TRACKS__)
...
if(__DUMP_COMPARE_SIGNAL_TRACKS__)
{
	fprintf(stderr, "Processing the %ld peaks for window %d with threshold %d, last peak was <%d-%d>\n", thr_peaks->size(), i_win, cur_win_threshold, last_peak_start, last_peak_end);
	...
	while(last_peak_list_index < (int)thr_peaks->size() && 
	...
	if(last_peak_list_index == (int)thr_peaks->size())
	{
		return;
	}
...
if(__DUMP_COMPARE_SIGNAL_TRACKS__)
	...
	n_chip_seq_reads = count_read_in_region(start, end, last_chip_seq_frag_i, chip_seq_fragments, enrichment_mapped_fragment_length);
	
	if(end - start > MAX_REG_EXT)
	{
		n_control_reads = (double)count_read_in_region(start, end, last_control_frag_i, control_fragments, enrichment_mapped_fragment_length);
	}
	else
	{
		n_control_reads = count_read_in_extended_region(start, end, last_control_frag_i, control_fragments, enrichment_mapped_fragment_length);
	}
...
int count_read_in_region(int start, int end, int& last_frag_list_i, vector<t_mapped_fragment*>* frag_list, int enrichment_mapped_fragment_length)
{
	int n_region_reads = 0;
        int last_added_index = -1000;
        int rep_cnt = 0;

        for(int i_frag = last_frag_list_i; i_frag < (int)frag_list->size(); i_frag++)
        {
                if(frag_list->at(i_frag)->base_index > end)
                {
                        break;
                }
                else if(frag_list->at(i_frag)->base_index + enrichment_mapped_fragment_length >= start)
                {
...
double count_read_in_extended_region(int start, int end, int& last_frag_list_i, vector<t_mapped_fragment*>* frag_list, int enrichment_mapped_fragment_length)
{
if(__DUMP_COMPARE_SIGNAL_TRACKS__)
	printf("Counting extended region tags: %d\n", last_frag_list_i);

	int cur_last_frag_list_i = last_frag_list_i;
	int n_region_reads_int = count_read_in_region(start, end, last_frag_list_i, frag_list, enrichment_mapped_fragment_length);
...
if(__DUMP_COMPARE_SIGNAL_TRACKS__)
	...
	while((frag_list->at(cur_last_frag_list_i)->base_index + enrichment_mapped_fragment_length) >= ext_start &&
		cur_last_frag_list_i > 0)
	{
		cur_last_frag_list_i--;
	}

	int n_ext_region_reads_int = count_read_in_region(ext_start, ext_end, cur_last_frag_list_i, frag_list, enrichment_mapped_fragment_length);
