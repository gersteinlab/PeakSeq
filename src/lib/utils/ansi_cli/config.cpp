...
t_config::t_config(char* config_fp)
{
	...
	this->vals = new vector<t_val_list*>();
	char cur_id[1000];
	char cur_val[2000];
	...
	while(fgets(cur_line, 5000, f_conf) != NULL)
	{
		...
		if(cur_line[0] != '#')
		{
			...
			else
			{
				...
				t_val_list* new_val_list = new t_val_list();

				// Add all the values to the value list.
				for(int i_val = 1; i_val < line_tokens->size(); i_val++)
				{
...
bool t_config::get_double_val(char* id, double& d_val)
{
	for(int i_id = 0; i_id < this->ids->size(); i_id++)
	{
...
bool t_config::get_str_val(char* id, char* val_buff)
{
        for(int i_id = 0; i_id < this->ids->size(); i_id++)
        {
                if(strcmp(this->ids->at(i_id), id) == 0)
...
bool t_config::get_int_val(char* id, int& int_val)
{
        for(int i_id = 0; i_id < this->ids->size(); i_id++)
        {
                if(strcmp(this->ids->at(i_id), id) == 0)
...
t_val_list* t_config::get_val_list(char* id)
{
        for(int i_id = 0; i_id < this->ids->size(); i_id++)
        {
                if(strcmp(this->ids->at(i_id), id) == 0)
