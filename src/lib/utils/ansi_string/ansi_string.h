...
public:
	...
	static char* substring(char* str, int i, int j);
	static void copy(char* dest_string, char* src_string);
	...
	static bool compare_substrings_ci(char* str, char* sub_str); // Check if the string contains sub_str as a substring in it, without regard to the case of letters.
	static void to_upper(char* string);
	static void clean_tokens(t_string_tokens* tokens);

	static bool starts_with(char* full_string, char* sub_str);
